...
const PrinciplesParodyRules: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-yellow-100 via-pink-100 to-blue-100 dark:from-yellow-900 dark:via-pink-900 dark:to-blue-900 py-8 sm:py-12 px-4">
      <div className="max-w-3xl mx-auto">
        <h1 className="text-3xl sm:text-4xl font-bold text-center mb-6 sm:mb-8 text-pink-800 dark:text-yellow-200 px-2">NoWindowShopping Parody Rules</h1>
        <p className="text-center text-base sm:text-lg mb-6 sm:mb-8 text-gray-700 dark:text-gray-300 px-2">A serious, professional, but comedic take on the rules of NoWindowShopping. Click a rule to expand. Search for keywords or rule numbers below.</p>
        <input
          type="text"
          value={search}
          ...
          {filtered.map((r, i) => (
            <div key={r.number} className="bg-white dark:bg-gray-800 rounded-xl shadow p-4 sm:p-6 border border-pink-100 dark:border-yellow-900">
              ...
              >
                ...
                <span className="text-gray-800 dark:text-gray-100 font-semibold text-sm sm:text-base leading-relaxed flex-1 min-w-0">{r.title}</span>
