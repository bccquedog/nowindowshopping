...
const PrinciplesHub: React.FC = () => (
  <div className="min-h-screen bg-gradient-to-br from-yellow-50 via-blue-50 to-indigo-100 dark:from-gray-900 dark:via-yellow-900 dark:to-blue-900 py-12 px-4">
    <div className="max-w-4xl mx-auto">
      <h1 className="text-4xl font-bold text-center mb-8 text-blue-900 dark:text-yellow-200">Principles of NoWindowShopping</h1>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        {principles.map((p) => (
          <Link
            key={p.title}
            to={p.to}
            className="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-8 flex flex-col items-center hover:scale-105 transition-transform border border-blue-100 dark:border-yellow-900"
          >
            <div className="text-5xl mb-4">{p.icon}</div>
            <h2 className="text-2xl font-bold mb-2 text-blue-800 dark:text-yellow-200">{p.title}</h2>
            <p className="text-gray-700 dark:text-gray-300 text-center">{p.description}</p>
          </Link>
        ))}
